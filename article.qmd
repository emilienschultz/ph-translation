...
- Plotly est l'un des seul package qui est spécifiquement tourné vers les représentations interactive. **Matplotlib** et **Pygal** ne fournissent que très peu de fonctionnalités interactives. **Bokeh**[^3] est aussi prévu pour l'interactivité et se place comme une alternatives viable.
...
[^4]: Si vous travaillez avec des notebooks Jupiter, il y a une bonne chance que certaines dépendances soient déjà installées. En revanche, si vous travaillez avec un nouvel environnement Python ou dans un IDE comme VS Code, il sera peut être nécessaire d'installer `ipykernel` (`pip install ipykernel`) et `nbformat` (`pip install nbformat`).

[^5]: Nous utiliserons aussi Numpy mais cette librairie est automatiquement téléchargé avec l'installation de Pandas.
...
[![**Figure 1.** Un diagramme en barres avec une intéractivité simple en utilisant **Plotly Express**. Si les lecteur.ice.s survolent les barres, on peut y voir apparaître des boîtes flottantes.](images/statique/fig_1.png)](https://axlmrin.github.io/ph-translation/images/interactif/fig_1.html)
...
```
[![**Figure 2.** Un diagramme en barres avec une intéractivité simple en utilisant **Plotly Express**. Cette visualisation est une variante de la **Figure 1** avec cette fois ci des couleurs et une légende interactive qui permet aux lecteur.ice.s d'isoler ou bien de retirer des données.](images/statique/fig_2.png)](https://axlmrin.github.io/ph-translation/images/interactif/fig_2.html)
...
[![**Figure 3.** Courbe avec une intéractivité simple en utilisant **Plotly Express**. Survoler les lignes révèle ne boîte flottante.](images/statique/fig_3.png)](https://axlmrin.github.io/ph-translation/images/interactif/fig_3.html)
...
```
[![**Figure 4.** Courbe avec une intéractivité simple en utilisant Plotly Express. Survoler les lignes révèle ne boîte flottante. Cette visualisation est une variante de la Figure 3 avec des polices d'écriture, couleurs et titre différent.](images/statique/fig_4.png)](https://axlmrin.github.io/ph-translation/images/interactif/fig_4.html)
...
[![**Figure 6.** Une mosaïque de 2 diagrammes en barres avec une intéractivité simple créée avec **Plotly Express** en distinguant les articles écrits par une majorité de femmes et ceux écrits par une majorité d'hommes. La légende intéractive permet aussi d'isoler, comparer ou retirer certaines disciplines.](images/statique/fig_6.png)](https://axlmrin.github.io/ph-translation/images/interactif/fig_6.html)
...
[![**Figure 7.** Diagramme en barres animé associé à une barre de défilement créés grâce à **Plotly Express**. Comme précédemment, les lecteur.ice.s peuvent survoler les barres pour faire apparaître des boîtes flottantes. Les lecteur.ice.s peuvent appuyer sur les boutons *play/pause* ou utiliser la barre de défilement pour naviguer à travers les années.](images/statique/fig_7.png)](https://axlmrin.github.io/ph-translation/images/interactif/fig_7.html)
...
Après avoir créé la vue initiale, nous pouvons utilisé la méthode `update_layout` à nouveau pour ajouter un menu déroulant.\\
...
[![**Figure 8.** Nuage de points avec un filtre intéractif sous la forme d'un menu déroulant créé grâce à **Plotly Express**. Cette figure contient une légende intéractive qui permet au lecteur d'isoler, comparer et retirer des données. De plus survoler des points permet de faire apparaître des boîtes flottantes.](images/statique/fig_8.png)](https://axlmrin.github.io/ph-translation/images/interactif/fig_8.html)
...
[![**Figure 9.** Diagramme en barres horizontal avec une intéractivité simple créé avec `Plotly Graph Objects`. Les lecteur.ices peuvent survoler les barres pour faire apparaître les boîtes flottantes.](images/statique/fig_9.png)](https://axlmrin.github.io/ph-translation/images/interactif/fig_9.html)
...
Mainteant créons la même visualisation avec `plotly.px` :
...
[![**Figure 10.** Diagramme en barres horizontal avec une interactivité simple créé avec `Plotly Express`. Les lecteurices peuvent survoler les barres pour faire apparaître les boîtes flottantes.](images/statique/fig_10.png)](https://axlmrin.github.io/ph-translation/images/interactif/fig_10.html)
...
[![**Figure 11.** Tableau contenant les données de nos articles et créé avec `Plotly Graph Object`. Les Lecteur.ices peuvent dérouller toutes les entrées du jeu de donnée comme iels le feraient dans un tableur.](images/statique/fig_11.png)](https://axlmrin.github.io/ph-translation/images/interactif/fig_11.html)
...
fig.add_trace(
    ...
    row = 1, col = 1 # Les paramètres row et col permettent de positionner la figure dans la bonne case
)
```
```{python}
#| echo: false
...
for discipline, df_discipline in proportion_d_articles_mentionnant_le_genre_par_annee.groupby("discipline") :
    fig.add_trace(
        ...
        row = 1, col = 2 # Les paramètres row et col permettent de positionner la figure dans la bonne case
    )
```
...
[![**Figure 13.** Une composition de figures avec 3 colonnes et une intéractivité simple créée avec le module `Plotly Graph Object`, et avec un diagramme en barres sur la gauche montrant le nombre d'articles mentionnant le genre par discipline, une courbe au centre montrant l'évolution de la proportion d'articles mentionnant le genre par discipline et une colonnes vide sur la droite. Les lecteur.ices peuvent survoler les barres pour faire apparaître les boîtes flottantes.](images/statique/fig_13.png)](https://axlmrin.github.io/ph-translation/images/interactif/fig_13.html)

#### Étape 5 : Ajouter la dernière figure (diagramme en boîte)

Nous n'avons pas encore exploré les diagrammes en boîte (*boxplot*), mais ils sont créés de la même manière que les autres figures et ont une intéractivité native similaire (survoler une boîte nous montrera la valeur minimum, maximum, médiane, et les interquartiles des données affichées).
...
# On ajoute le deuxième diagramme en boîte puisqu'on a 2 groupes distincts pour les articles avec et sans écriture inclusive
fig.add_trace(
    go.Box(
        y = pourcentage_d_auteurice_femme_si_inclusif.\
...
[![**Figure 14.** Une composition de figures avec 3 colonnes et une intéractivité simple créée avec le module `Plotly Graph Object`, et avec un diagramme en barres sur la gauche montrant le nombre d'articles mentionnant le genre par discipline, une courbe au centre montrant l'évolution de la proportion d'articles mentionnant le genre par discipline et deux boîtes en boîte représentant la distribution de la part d'autrices dans les articles selon s'il est publié en écriture inclusive ou non. Les lecteur.ices peuvent survoler les barres pour faire apparaître les boîtes flottantes.](images/statique/fig_14.png)](https://axlmrin.github.io/ph-translation/images/interactif/fig_14.html)
...
fig.update_layout(
    ...
    # title_x = 0.5 # Positionnement du titre principal au centre de la visualisation (note : le paramètre title_x ne prend que des entiers (integers) ou des réels (floats))
    xaxis1_title_text = "Nombre d'articles mentionnant le genre", # ajout d'un titre d'axe pour l'absisse de la première figure
    yaxis1_title_text = "Catégorie du journal", # ajout d'un titre d'axe pour les ordonnées de la première figure
    ...
    height = 650 # Ajuste la taille de la visualisation  - pas toujours nécessaire mais peut s'avérer utilse si les figures sont publiées sur internet
)
``` 
...
[![**Figure 15.** Une composition de figures avec 3 colonnes et une intéractivité simple créée avec le module `Plotly Graph Object`, et avec un diagramme en barres sur la gauche montrant le nombre d'articles mentionnant le genre par discipline, une courbe au centre montrant l'évolution de la proportion d'articles mentionnant le genre par discipline et deux diagrammes en boîte représentant la distribution de la part d'autrices dans les articles selon s'il est publié en écriture inclusive ou non. Cette visualisation est une variante de la figure 14 avec une personalisation avancée.](images/statique/fig_15.png)](https://axlmrin.github.io/ph-translation/images/interactif/fig_15.html)
#### Étape 7 : Ajout d'annotations aux courbes

Puisque la légende à été retirée, il est impossible de sitinguer une discipline des autres. Nous pouvons utiliser la méthode `.update_layout` pour ajouter des flèches pointant vers chaque ligne avec une annotation : 
...
fig.update_layout(
    # Entre comme paramètre la list de dictionnaires où chaque dictionnaire représente une annotation
    annotations = [
        ...
        dict(
            ...
            x = 2005, y = 0.897,
            # Spécifie fans quel référentiel on se place, ici comme on annote la figure n°2 on donne comme référence x2, y2
            xref = "x2", yref = "y2",
            # Permet de spécifier la longueur de la flèche, et donc du déport du point
            ax = 30, ay = 100,
            text = "Études de Genre",
            showarrow = True, # Utilisez False si vous ne voullez pas de la tête de flèche dans l'annotation
...
fig.update_layout(width = 1200,height = 400)
fig.write_image("./images/statique/fig_16.png")
save_html(fig, "./images/interactif/fig_16.html")
```

[![**Figure 16.** Une composition de figures avec 3 colonnes et une intéractivité simple créée avec le module `Plotly Graph Object`, et avec un diagramme en barres sur la gauche montrant le nombre d'articles mentionnant le genre par discipline, une courbe au centre montrant l'évolution de la proportion d'articles mentionnant le genre par discipline et deux diagrammes en boîte représentant la distribution de la part d'autrices dans les articles selon s'il est publié en écriture inclusive ou non. Cette visualisation est une variante de la figure 15 des annotations pour repérer les courbes du sous-figure du milieu](images/statique/fig_16.png)](https://axlmrin.github.io/ph-translation/images/interactif/fig_16.html)
...
fig.add_annotation(
    dict(
        ...
        showarrow=False,
        text=(
            "Nombre d'articles publiés par discipline (gauche);<br>"
            "proportion d'articles mentionnant le genre à travers les années (centre);<br>"
            "distribution de la part de femmes autrices lorsque l'article est publié en écriture inclusive (droite)."),
        textangle=0,  # Option pour changer l'orientation de l'écriture, utile pour la gestion de l'espace
        xanchor="center",
        xref="paper",  # Régler xref et yref à 'paper' pour que les valeurs de x et y soient des coordonées absolues
        yref="paper",
    )
)
```
...
fig.update_layout(width = 1200,height = 400)
fig.write_image("./images/statique/fig_17.png")
save_html(fig, "./images/interactif/fig_17.html")
```

[![**Figure 17.** Une composition de figures avec 3 colonnes et une intéractivité simple créée avec le module `Plotly Graph Object`, et avec un diagramme en barres sur la gauche montrant le nombre d'articles mentionnant le genre par discipline, une courbe au centre montrant l'évolution de la proportion d'articles mentionnant le genre par discipline et deux diagrammes en boîte représentant la distribution de la part d'autrices dans les articles selon s'il est publié en écriture inclusive ou non. Cette visualisation est une variante de la figure 16 avec des annotations supplémentaires sous les figures.](images/statique/fig_17.png)](https://axlmrin.github.io/ph-translation/images/interactif/fig_17.html)

# Visualiser et Exporter les visualisations

Dans les sections précédentes de la leçon, nous avons vu comment créer et modifier les visualisations intéractives avec **Plotly Express** et **Plotly Graph Objects**. Nous allons maintenant apprendre comment faire apparaître les visualisations et les exproter pour les publier ou les partager.\\
...
## Faire apparaître la visualisation

Comme nous l'avons vu tout le long de cette leçon, la méthode `.show()` peut être utilisée pour faire apparaître la figure. Par défaut, cette méthode utilise le générateur d'image **Plotly** qui fournit l'intéractivité native :
```{python}
#| echo: true
#| eval: false
...
fig.update_layout(width = 800,height = 600)
fig.write_image("./images/statique/fig_18.png")
save_html(fig, "./images/interactif/fig_18.html")
```

[![**Figure 18.** Reproduction de la figure 3, illustrant la fonction `fig.show()`.](images/statique/fig_18.png)](https://axlmrin.github.io/ph-translation/images/interactif/fig_18.html)

## Export des visualisations

Les figures **Plotly** peuvent être exportées en version statique (donc sans interactivité) ou en version interactive. Les visualisations intéractives sont utiles pour les sites de recherches et certaines publications numériques, tandis que les versions statiques sont plus appropriées aux publications imprimées.
...
**Plotly** fournit de nombreuses options pour exporter des images avec pixels (`.png` ou `.jpg`) et images vectorielles (`.pdf` ou `.svg`). Pour cela, il suffit d'utiliser la méthode `write_image()` et spécifier quel type d'image nous souhaitons dans le nom du fichier :
